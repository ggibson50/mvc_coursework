...
GO
PRINT N'Dropping unnamed constraint on [dbo].[order_form]...';


GO
ALTER TABLE [dbo].[order_form] DROP CONSTRAINT [FK__order_for__order__3B75D760];


GO
PRINT N'Dropping unnamed constraint on [dbo].[parts]...';
...
GO
ALTER TABLE [dbo].[parts] DROP CONSTRAINT [FK__parts__laborId__3D5E1FD2];
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[order_form])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_order_form] ON;
        ...
        SELECT [orderNum],
               ...
               [model]
        FROM   [dbo].[order_form];
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_order_form] OFF;
    END
...
GO
PRINT N'Starting rebuilding table [dbo].[parts]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_parts] (
    [orderNum]   INT              IDENTITY (1, 1) NOT NULL,
    [laborId]    UNIQUEIDENTIFIER NOT NULL,
    [estimate]   DECIMAL (18)     NOT NULL,
    [quantity]   INT              NOT NULL,
    [nameOfPart] VARCHAR (25)     NULL,
    [costOfPart] DECIMAL (10, 2)  NULL,
    [partNum]    INT              NOT NULL,
    PRIMARY KEY CLUSTERED ([partNum] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[parts])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_parts] ON;
        INSERT INTO [dbo].[tmp_ms_xx_parts] ([partNum], [orderNum], [laborId], [estimate], [quantity], [nameOfPart], [costOfPart])
        SELECT   [partNum],
                 [orderNum],
                 [laborId],
                 [estimate],
        ...
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_parts] OFF;
    END

DROP TABLE [dbo].[parts];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_parts]', N'parts';
...
ALTER TABLE [dbo].[labor_details] WITH NOCHECK
    ADD FOREIGN KEY ([partNum]) REFERENCES [dbo].[parts] ([partNum]);
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.order_form'), OBJECT_ID(N'dbo.parts'), OBJECT_ID(N'dbo.labor_details'))
               AND [type] IN (N'F', N'C')
