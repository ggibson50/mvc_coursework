...
IF EXISTS (select top 1 1 from [dbo].[parts])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT

GO
PRINT N'Altering [dbo].[parts]...';


GO
ALTER TABLE [dbo].[parts] DROP COLUMN [checked];
...
GO
ALTER TABLE [dbo].[parts]
    ADD PRIMARY KEY CLUSTERED ([partNum] ASC);
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.labor_details'))
               AND [type] IN (N'F', N'C')
